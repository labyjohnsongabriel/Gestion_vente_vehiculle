...
const BACKEND_URL = "http://localhost:5000"; // Ajoutez ceci en haut du fichier
...
const PieceList = () => {
  ...
  // Filtrage et tri alphabétique des pièces
  const filteredPieces = pieces
    .filter((piece) => {
      ...
      const matchesCategory =
        filterCategory === "" ||
        piece.category_id?.toString() === filterCategory;
                      ...
                      <PremiumTableRow key={piece.id}>
                        <TableCell>
                          <Avatar
                            src={
                              piece.image
                                ? piece.image.startsWith("/uploads/")
                                  ? `http://localhost:5000${piece.image}`
                                  : piece.image
                                : undefined
                            }
                            alt={piece.name || "Pièce"}
                            sx={{ bgcolor: "#667eea", color: "white" }}
                          >
                            {!piece.image && piece.name ? (
                              piece.name.charAt(0).toUpperCase()
                            ) : !piece.image && !piece.name ? (
                              <img
                                src="/default-part.png"
                                alt="default"
                        ...
                        <TableCell>
                          <Chip
                            label={piece.categorie || "Non renseigné"}
                            color="secondary"
                          />
                        </TableCell>
                        <TableCell>
                          <Chip
                            label={piece.fournisseur || "Non renseigné"}
                            color="primary"
                          />
                        </TableCell>
              ...
              <Grid container spacing={3} sx={{ mt: 1 }}>
                <Grid item xs={12} md={4}>
                  <Avatar
                    src={
                      selectedPiece?.image
                        ? selectedPiece.image.startsWith("http")
                          ? selectedPiece.image
                          : `${BACKEND_URL}${selectedPiece.image}`
                        : "/default-part.png"
                    }
                    sx={{
                      width: "100%",
...
const PieceList = () => {
  ...
  const fetchClients = async () => {
    try {
      ...
      showSnackbar("Impossible de charger les clients", "error");
    }
  };
  ...
  // Gestion des ventes
  const handleSell = (piece) => {
    if (piece.stock_quantity <= 0) {
      showSnackbar("Stock insuffisant pour cette pièce", "error");
      return;
    }
    setSelectedPiece(piece);
    setQuantityToSell(1);
  ...
  const addToCart = () => {
    const quantity = parseInt(quantityToSell) || 0;
    
    if (quantity <= 0 || quantity > selectedPiece.stock_quantity) {
    ...
    if (existingItemIndex >= 0) {
      ...
      updatedCart[existingItemIndex].total = parseFloat((updatedCart[existingItemIndex].price * updatedCart[existingItemIndex].quantity).toFixed(2));
      setCart(updatedCart);
    } else {
      setCart([
        ...cart,
        {
          ...selectedPiece,
          quantity: quantity,
          total: parseFloat((selectedPiece.price * quantity).toFixed(2)),
        },
      ]);
    }

    setSaleStep(1);
    showSnackbar("Pièce ajoutée au panier");
  };
  ...
  const processSale = async () => {
    if (!selectedClient) {
      showSnackbar("Veuillez sélectionner un client", "error");
      return;
    }
    ...
    try {
      ...
      const saleData = {
        ...
        items: cart.map((item) => ({
          piece_id: parseInt(item.id),
          quantity: parseInt(item.quantity) || 0,
          price: parseFloat(item.price) || 0,
        })),
        total: parseFloat(cart.reduce((sum, item) => sum + (parseFloat(item.total) || 0), 0).toFixed(2)),
        user_id: parseInt(userId),
      };

      // Validation supplémentaire
      if (saleData.items.some(item => isNaN(item.quantity) || isNaN(item.price) || item.quantity <= 0)) {
        showSnackbar("Données de vente invalides", "error");
        return;
      }

      if (isNaN(saleData.total) || saleData.total <= 0) {
        showSnackbar("Le total de la vente est invalide", "error");
        return;
      }

      await axios.post(VENTES_API_URL, saleData, {
        headers: { Authorization: `Bearer ${token}` },
      });
      ...
      const updatedPieces = pieces.map((piece) => {
        ...
        if (cartItem) {
          return {
            ...piece,
            stock_quantity: piece.stock_quantity - (parseInt(cartItem.quantity) || 0),
          };
        }
        return piece;
      });
      ...
      setSelectedClient("");
      setSaleOpen(false);
      showSnackbar("Vente enregistrée avec succès");
    } catch (error) {
      console.error("Erreur lors de l'enregistrement de la vente:", error);
      showSnackbar("Échec de l'enregistrement de la vente", "error");
    }
  };
    ...
    <Fade in timeout={600}>
      <Box sx={{ p: { xs: 1, sm: 3 } }}>
                        ...
                        <TableCell align="right">
                          <Tooltip title="Vendre">
                            <IconButton
                              ...
                              disabled={piece.stock_quantity <= 0}
                            >
                              <PointOfSale
                                color={
                                  piece.stock_quantity <= 0
        ...
        >
          ...
          >
            <Typography variant="h5">Processus de Vente</Typography>
            <IconButton
              onClick={() => {
                setSaleOpen(false);
                setCart([]);
          ...
          <Stepper activeStep={saleStep} sx={{ p: 3 }}>
            <Step>
              <StepLabel>Sélection de la quantité</StepLabel>
            </Step>
            <Step>
              <StepLabel>Panier</StepLabel>
            </Step>
            <Step>
          ...
          <DialogContent dividers>
            {saleStep === 0 && selectedPiece && (
              <Grid container spacing={3}>
                <Grid item xs={12}>
                  <Typography variant="h6" gutterBottom>
                    Vente de: {selectedPiece.name}
                  </Typography>
                ...
                <Grid item xs={12} sm={6}>
                  <TextField
                    ...
                    onChange={(e) => {
                      ...
                      setQuantityToSell(Math.min(value, selectedPiece.stock_quantity));
                    }}
                    inputProps={{
                      min: 1,
                      max: selectedPiece.stock_quantity,
                    }}
                    helperText={`Stock disponible: ${selectedPiece.stock_quantity}`}
                  />
                </Grid>
                ...
                <Grid item xs={12} sm={6}>
                  <Card variant="outlined">
                    <CardContent>
                      <Typography variant="h6" color="primary">
                        Total: {(selectedPiece.price * quantityToSell).toFixed(2)} €
                      </Typography>
                      <Typography variant="body2">
                        Prix unitaire: {selectedPiece.price} €
                      </Typography>
                    </CardContent>
                  </Card>
                </Grid>
            ...
            {saleStep === 1 && (
              <Box>
                  ...
                  <List>
                    {cart.map((item) => (
                      <ListItem key={item.id} divider>
                        <ListItemText
                          primary={item.name}
                          secondary={`Réf: ${item.reference} | ${item.quantity} x ${item.price} €`}
                        />
                ...
                <Box sx={{ display: "flex", justifyContent: "space-between" }}>
                  <Typography variant="h6">Total panier:</Typography>
          ...
          <DialogActions sx={{ p: 3, gap: 1 }}>
            ...
            {saleStep === 0 && (
              <Button
                onClick={addToCart}
                variant="contained"
                disabled={quantityToSell <= 0 || quantityToSell > selectedPiece.stock_quantity}
            ...
            <Card>
              <CardContent>
                  ...
                  <Grid item xs={12} md={4}>
                    <Card variant="outlined" sx={{ textAlign: 'center', p: 2 }}>
                      <Typography variant="h4" color="primary">
                        {pieces.reduce((total, piece) => total + (piece.stock_quantity || 0), 0)}
                      </Typography>
                  ...
                  <Grid item xs={12} md={4}>
                    <Card variant="outlined" sx={{ textAlign: 'center', p: 2 }}>
                      <Typography variant="h4" color="secondary">
                        {pieces.filter(p => p.stock_quantity > 0).length}
                      </Typography>
                      <Typography variant="body2" color="text.secondary">
                      ...
                      <Typography variant="h4" color="success.main">
                        {pieces
                          .reduce((total, piece) => {
                            ...
                            const quantity = parseInt(piece.stock_quantity) || 0;
                ...
                <Box sx={{ mt: 3 }}>
                  ...
                  {categories.map(category => {
                    const categoryPieces = pieces.filter(p => p.category_id === category.id);
                    const totalValue = categoryPieces.reduce((total, piece) => {
                      const price = parseFloat(piece.price) || 0;
                      const quantity = parseInt(piece.stock_quantity) || 0;
                      return total + (price * quantity);
                    }, 0);
                    
                    const totalQuantity = categoryPieces.reduce((total, piece) => 
                      total + (parseInt(piece.stock_quantity) || 0), 0);
                    
                    return (
                      <Box key={category.id} sx={{ mb: 1 }}>
                        <Typography variant="body2">
                          {category.name}: {totalQuantity} pièces - {totalValue.toFixed(2)} €
                        </Typography>
                      </Box>
                    );
                  })}
